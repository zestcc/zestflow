...
onMounted(async () => {
  await fetchApps()
  const executionIdFromQuery = route.query.executionId as string | undefined
  if (executionIdFromQuery) {
    query.executionId = executionIdFromQuery
  }
  await fetchList()
  if (executionIdFromQuery) {
    await showDetail({ executionId: executionIdFromQuery } as ExecutionTrace)
  }
})
