 ...
 */
@RestController
@RequestMapping("/demo/playground")
@ConditionalOnProperty(prefix = "zestflow.demo", name = "enabled", havingValue = "true", matchIfMissing = false)
@RequiredArgsConstructor
public class DemoPlaygroundController {

    private final DemoPlaygroundService playgroundService;
    ...
    public Result<DemoSceneVO> getSceneInfo(@PathVariable String sceneCode) {
        DemoSceneVO vo = playgroundService.getSceneInfo(sceneCode);
        if (vo == null) {
     ...
     */
    @PostMapping("/history")
    public Result<IPage<DemoRecordVO>> queryHistory(@RequestBody DemoRecordQueryDTO dto) {
    ...
    public Result<DemoRecordVO> getHistoryDetail(@PathVariable Long id) {
