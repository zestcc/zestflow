...
/**
 * 通用链质量门禁：检测「单节点黑盒」等不合格草稿，触发 AI 自行扩写重试（非硬编码业务模板）。
 */
public final class AiChainQualityGate {
        ...
        if (businessNodes <= 2) {
            return QualityResult.reject(
                    "业务链过于简化（仅 " + businessNodes + " 个业务节点）。"
                            + "请自行对标业界同类成熟方案的主路径，拆分为解析、校验、分支、核心操作、响应等独立节点，"
                            + "并包含必要的异常分支；目标 90% happy path 可 Playground 试跑。");
        }
        if (isBlackBoxChain(chainDataJson, userMessage)) {
            return QualityResult.reject(
                    "检测到「单节点黑盒」（节点 label 与需求笼统同名）。"
                            + "请按验收标准拆步，每节点单一职责，不得用一个节点代替整条业务链。");
        }
        if (!hasBranchWhenNeeded(chainDataJson) && businessNodes <= 4) {
            return QualityResult.reject(
                    "该类业务通常需要规则分支（如重复校验、状态判断），当前链缺少 CONDITION 分叉或失败路径。"
                            + "请参考业界方案补全分支与错误响应节点。");
        }
        return QualityResult.ok();
    }
