...
-- 2026-05-29：链执行事件表（Executor 发射 → Collector 采集落库）
CREATE TABLE IF NOT EXISTS `chain_event` (
    `id`            BIGINT       NOT NULL AUTO_INCREMENT  COMMENT '自增主键',
    `event_id`      VARCHAR(64)  NOT NULL                 COMMENT '事件全局唯一 ID（UUID）',
    `event_type`    VARCHAR(32)  NOT NULL                 COMMENT '事件类型：CHAIN_STARTED/NODE_STARTED/NODE_COMPLETED/NODE_FAILED/CHAIN_COMPLETED/CHAIN_FAILED/CHAIN_TIMEOUT',
    ...
    `executor_id`   VARCHAR(128) DEFAULT NULL             COMMENT '执行器 ID（moduleCode@host:port）',
    `app_name`      VARCHAR(64)  DEFAULT NULL             COMMENT '应用名（分组标识）',
