...
# ==================================
# ZestFlow Demo — 启动脚本
# 可调参数见 config/start-demo.env
#
#   ./start-demo.sh
#   ./config/application.yml  或 ./application-demo.yml（平铺亦可，自动识别 profile）
#   ./config/start-demo.env  或 ./start-demo.env
#   ./zestflow-demo-0.1.0.jar   （多版本时取版本号最大）
#   ./log/
# ==================================
...
_resolve_paths() {
  ...
  for dir in "$SCRIPT_DIR/config" "$SCRIPT_DIR"; do
    ...
    if compgen -G "${dir}/application*.yml" > /dev/null 2>&1 \
        || [ -f "${dir}/start-demo.env" ]; then
      [ -z "$ENV_FILE" ] && [ -f "${dir}/start-demo.env" ] && ENV_FILE="${dir}/start-demo.env"
  ...
  [ -z "$ENV_FILE" ] && ENV_FILE="$SCRIPT_DIR/config/start-demo.env"
  [ ! -f "$ENV_FILE" ] && [ -f "$SCRIPT_DIR/start-demo.env" ] && ENV_FILE="$SCRIPT_DIR/start-demo.env"
}
  ...
  if [ -z "${SPRING_PROFILE:-}" ]; then
    if [ -n "$CONFIG_FILE" ]; then
      ...
      if [ -z "${SPRING_PROFILE:-}" ]; then
        ...
        _log "         请在 start-demo.env 中设置 SPRING_PROFILE=demo|prod|..."
        return 1
      fi
...
_preflight() {
  ...
  if [ -x "$JAVA_BIN" ]; then
    ...
    _log "  [FAIL] 未找到 java"
    _log "         1) 在 config/start-demo.env 设置："
...
目录（相对脚本位置）:
  ...
  ./config/start-demo.env       启动/JVM；SPRING_PROFILE=prod
