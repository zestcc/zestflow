# Apply IP demo V1 DB constraints (reads password from application-local.yml)
param(
...
$env:MYSQL_PWD = $pwd
$migration = Join-Path $Root 'zestflow-admin\src\main\resources\db\migration\V3__ip_demo_tenant_constraints.sql'
if (-not (Test-Path $MysqlBin)) { Write-Error "mysql not found: $MysqlBin" }
& $MysqlBin -h $MysqlHost -u $MysqlUser --default-character-set=utf8mb4 -e "source $($migration -replace '\\','/')" 2>&1 | Out-Host
Remove-Item Env:MYSQL_PWD -ErrorAction SilentlyContinue
Write-Host 'Done: V3 ip demo constraints'
