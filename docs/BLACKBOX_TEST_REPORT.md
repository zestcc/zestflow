...
| 项目 | 内容 |
|------|------|
| 版本 | V1.0-SNAPSHOT（本地联调） |
| 测试日期 | 2026-06-02 |
| 测试类型 | 黑盒（Black-box）+ 局部压力探测 |
| 执行人 | 自动化脚本 + 人工复核 |
| 原始数据 | `scripts/blackbox/results/blackbox-20260602-155833.json` |
...
| P-02 | Playground 全场景顺序回归 | 1 | 1 run | 33 场景全部 200 |
...
4. **33 个试验场种子场景**：2 个已实测，其余已列入矩阵与批量脚本，建议发版前跑完全量回归。
...
| 门禁项 | 标准 |
|--------|------|
| 全场景试验场回归 | 33/33 返回 code=200（或预期业务失败） |
