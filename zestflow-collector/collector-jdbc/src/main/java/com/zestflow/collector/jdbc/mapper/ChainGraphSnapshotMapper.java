...
public interface ChainGraphSnapshotMapper extends BaseMapper<ChainGraphSnapshotPO> {
     ...
     */
    @Select("SELECT MAX(version) FROM chain_graph_snapshot WHERE chain_code = #{chainCode} FOR UPDATE")
    Integer selectMaxVersionForUpdate(@Param("chainCode") String chainCode);
    ...
    @Update("UPDATE chain_graph_snapshot s " +
            ...
            "  AND s.STATUS = 1 " +
            "  AND e.id IS NULL")
    int deprecateUnreferenced(@Param("chainCode") String chainCode);
}
